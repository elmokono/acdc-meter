...
void updateMeter(Meter &m) {
  if (millis() - m.lastCalc < 1000) return;
  m.lastCalc += 1000;
...
void handleData() {
  server.send(200, "application/json",
              "{"
              "\"A\":{\"w\":"
                + String(A.watts, 1) + ",\"kwh\":" + String(totalKwh(A), 3) + "},"
                                                                              "\"B\":{\"w\":"
                + String(B.watts, 1) + ",\"kwh\":" + String(totalKwh(B), 3) + "}"
                                                                              "}");
}
...
/* ================= UI ================= */
void handleUI() {
  server.send(200, "text/html", "<h1>Energy Monitor</h1>");
}
